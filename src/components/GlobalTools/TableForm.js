...
class TableForm extends PureComponent {
  ...
  onSelectChange = (selectedRowKeys, selectedRows) => {
    ...
    const newArr = selectedRows.map(val => {
      ...
      let value = Object.assign({},val, defaultValue)
      console.log(value)
      return value
    })
    this.setState({
  ...
  newSingleValue = () => {
    ...
    if (member === 'member') {
      ...
      this.index += 1;
      console.log(mColumns,newValue,newData, 'member')
      this.setState({ data: newData });
    } else {
  ...
  handleFieldChange(e, val, key, mode) {
    ...
    const { data } = this.state;
    console.log( e.target,data,' e.target')
    const newData = data.map(item => ({ ...item }));
    const target = this.getRowByKey(key, newData);
    if (target) {
  ...
  handSelectInputChange(e, val, record) {
      console.log(e, val, record, 'e, val, record')
    const fieldName = val.dataIndex
    ...
    console.log(selectArr,childKeyValue, e, fieldName, key,'childKeyValue')
    const { data } = this.state;
    console.log( e.target,data,' e.target')
    const newData = data.map(item => ({ ...item }));
    const target = this.getRowByKey(key, newData);
  ...
  render() {
    const { loading, data, modalVisible, reChildFlag, selectedRowKeys } = this.state;
    ...
    const newcColumns = mColumns.map(val => {
      ...
      if (val.type === 'input') {
        ...
        console.log(val.selectArr, 'val.selectArr')
        const selectObj = {
          key: val.dataIndex,
          // width: `${100 / (len + 2)}%`,
          render: (text, record) => {
